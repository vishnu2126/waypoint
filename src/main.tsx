...
const roadmapSeo: Record<string, { title: string; description: string }> = Object.fromEntries(roadmaps.map(route => {
  const subjects = route.skills.slice(0, 5).map(skill => skill.name).join(', ');
  const hours = route.skills.reduce((total, skill) => total + skill.hours, 0);
  const focus: Record<string, string> = {
    frontend: 'HTML, CSS, JavaScript, React, accessibility and web performance',
    backend: 'Java, Spring Boot, APIs, SQL, distributed systems and service reliability',
    'full-stack': 'HTML, JavaScript, React, Node.js, APIs, databases and deployment',
    database: 'SQL, data modeling, PostgreSQL, query tuning, replication and recovery',
    cloud: 'Linux, networking, IAM, containers, Kubernetes, Terraform and cloud operations',
    devops: 'Linux, Docker, Kubernetes, CI/CD, infrastructure as code and observability',
    programming: 'variables, functions, data structures, algorithms, testing and software design',
  };
  const routeFocus = focus[route.id] ?? subjects.toLowerCase();
  return [route.id, {
    title: `${route.name} Roadmap 2026 | Waypoint`,
    description: `${route.blurb} Learn ${routeFocus} through ${route.skills.length} practical modules and approximately ${hours} hours of structured study.`,
  }];
}));
...
const publicPath = (pathname: string, skill?: Skill) => pathname === '/' || pathname === '/home' || pathname === '/explore' ||
  (pathname.startsWith('/roadmap/') && roadmaps.some(route => roadmapPath(route.id) === pathname)) ||
  (pathname.startsWith('/skill/') && Boolean(skill));
function SeoHead({ pathname, roadmap, skill }: { pathname: string; roadmap: Roadmap; skill?: Skill }) {
  useEffect(() => {
    ...
    const routeMetadata = roadmapSeo[roadmap.id];
    const title = isRoadmap && routeMetadata ? routeMetadata.title : isSkill && skill ? `${skill.name} Learning Module | Waypoint` : pathname === '/explore' ? 'Explore Developer Roadmaps | Waypoint' : 'Waypoint - Developer Navigation Engine';
    const description = isRoadmap && routeMetadata ? routeMetadata.description : isSkill && skill ? `Learn ${skill.name} with a structured Waypoint module covering ${skill.topics?.slice(0, 3).join(', ').toLowerCase() ?? 'concepts, practice, projects, and proof of understanding'}.` : pathname === '/explore' ? 'Compare structured developer roadmaps for frontend, backend, full stack, Java, databases, cloud, DevOps, programming, Python, TypeScript, QA, and security.' : 'Waypoint helps developers choose a destination, follow a structured roadmap, and find the next best learning move.';
    const indexable = publicPath(pathname, skill);
    const canonicalPath = pathname === '/' ? '/home' : pathname;
    const canonical = indexable ? `${siteUrl}${canonicalPath}` : `${siteUrl}/home`;
    ...
    if (indexable) {
      const graph: Record<string, unknown>[] = [{ '@type': isRoadmap ? 'CollectionPage' : pathname === '/explore' ? 'CollectionPage' : 'WebSite', '@id': canonical, url: canonical, name: title, description }];
      if (isRoadmap) {
        graph.push({ '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: `${siteUrl}/home` }, { '@type': 'ListItem', position: 2, name: 'Explore roadmaps', item: `${siteUrl}/explore` }, { '@type': 'ListItem', position: 3, name: roadmap.name, item: canonical }] });
        graph.push({ '@type': 'ItemList', name: `${roadmap.name} curriculum`, numberOfItems: roadmap.skills.length, itemListElement: roadmap.skills.slice(0, 10).map((item, index) => ({ '@type': 'ListItem', position: index + 1, name: item.name })) });
      }
